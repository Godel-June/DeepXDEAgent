...
# openai_api_key = "sk-e7da98d7c23242509ece68a221e02348"
# openai_api_base = "https://api.deepseek.com"

url = "http://120.220.102.26:38001/v1/chat/completions"
headers = {
    "Authorization": "Bearer zidong-ai-deepseek-r1-7Yq9pL3RfTvW2zAmN1bXc6Zx8KjH5GhD4",
...
# TODO: 通过os.getenv获取API Key，或者传参实现

def request_llm(system_prompt: str, user_prompt: str):
    """
    请求语言模型生成回复，包括思考过程和代码，但是最终通过提取函数来获取可执行代码
    """
    ...
    return extract_program(response.choices[0].message.content.strip())


def request_llm_local(system_prompt: str, user_prompt: str):
    """
    请求本地部署的语言模型生成回复，包括思考过程和代码，但是最终通过提取函数来获取可执行代码
    """
    data = {
        "model": "deepseek_r1",
        "stream": False,
    ...
    return extract_program(message)


# 工具 1: 定义PDE
def define_pde(equation: str):
    """
    定义偏微分方程。
    参数:
    ...
    """
    pde_def_prompt_path = os.path.join(os.path.dirname(__file__), "prompt", "pde_def_prompt.txt")
...
# 工具 2: 定义解析解
def define_reference_solution(solution: str):
    """
    定义参考解。
    参数:
    ...
    """
    # 使用相对路径加载提示文件
    ref_sol_def_prompt_path = os.path.join(os.path.dirname(__file__), "prompt", "ref_sol_def_prompt.txt")
...
# 工具 4：定义初始条件
def define_initial_condition(geomtime, ic_description: str):
    """
        定义初始条件。
        参数:
            geomtime: 空间-时间几何对象
    ...
    """
    ic_def_prompt_path = os.path.join(os.path.dirname(__file__), "prompt", "ic_def_prompt.txt")
...
# 工具 5：定义边界条件
def define_boundary_condition(geomtime,  bc_description: str):
    """
    ...
    """
    bc_def_prompt_path = os.path.join(os.path.dirname(__file__), "prompt", "bc_def_prompt.txt")
